...
export default function LeadDetailsModal({ isOpen, onClose, client, onClientUpdated, onClientCreated, lang = 'es' }: LeadDetailsModalProps) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (onClientUpdated) {
                // We'll use a special flag or just handle it as "removed"
                // For this implementation, we re-fetch in parent anyway if we want total sync
                // but let's notify the parent to remove it from state
...
return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-2xl h-full sm:h-auto bg-[#0f1014]/90 border border-white/10 sm:rounded-2xl shadow-2xl overflow-hidden glass-card animate-in fade-in zoom-in-95 duration-200 flex flex-col">
            ...
            <div className="p-4 border-t border-white/5 bg-white/5 flex flex-col sm:flex-row justify-between gap-3">
                <div className="flex gap-2 order-2 sm:order-1">
                    {!isNew && !isEditing && (
                        <button
                            ...
                            className="px-4 py-2 rounded-lg text-red-400 hover:bg-red-400/10 transition-colors text-sm font-medium border border-red-400/20"
                        >
                ...
                <div className="flex gap-2 order-1 sm:order-2">
                    ...
                    >
                        {isEditing ? (saving ? (lang === 'es' ? "Guardando..." : "Saving...") : (lang === 'es' ? "Guardar Cambios" : "Save Changes")) : (lang === 'es' ? "Cerrar" : "Close")}
                    </button>
                </div>
            </div>
                    ...
                    <div className="glass-card p-8 border-red-500/30 max-w-sm w-full text-center">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button
                                ...
                                className="flex-1 py-3 bg-red-500 text-white rounded-xl disabled:opacity-30 disabled:grayscale transition-all hover:bg-red-600"
                            >
